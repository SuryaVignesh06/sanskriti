...
import './chunks/astro-designed-error-pages_DJzvrv3J.mjs';
import 'piccolore';
import './chunks/astro/server_3QCLOVPR.mjs';
import 'clsx';
import { s as sequence } from './chunks/render-context_pEyBIhL_.mjs';
import { AsyncLocalStorage } from 'node:async_hooks';
import { headlessNode } from '@wix/headless-node';
import { createClient, OAuthStrategy, TokenRole, AppStrategy } from '@wix/sdk';
...
import { W as WIX_CLIENT_PUBLIC_KEY, a as WIX_CLIENT_SECRET, b as WIX_CLIENT_INSTANCE_ID } from './chunks/server_BMj9HAzV.mjs';
...
//#region src/utils/authAsyncLocalStorage.ts
const authAsyncLocalStorage = new AsyncLocalStorage();

//#endregion
//#region src/utils/host-utils.ts
function createHost(request, essentials) {
	return headlessNode.host({
		essentials,
		req: request
	});
}

//#region src/runtime/backend/hostProxy.ts
const defaultHost = createHost();
const resolveHostForCurrentRequest = () => {
	return authAsyncLocalStorage.getStore()?.sdkHost ?? defaultHost;
};
const hostProxy = new Proxy({}, { get(target, prop) {
	const value = resolveHostForCurrentRequest()[prop];
	if (typeof value !== "function") return value;
	return function(...args) {
		const anotherHost = resolveHostForCurrentRequest();
		return anotherHost[prop].apply(anotherHost, args);
	};
} });

//#endregion
//#region src/runtime/backend/contextualClient.ts
const contextualClient = createClient({
	auth: () => {
		const store = authAsyncLocalStorage.getStore();
		if (store?.auth == null) throw new Error("Cannot find authentication on `AsyncLocalStorage` instance");
		return store.auth;
	},
	host: hostProxy
});
...
//#endregion
//#region src/runtime/backend/elevatedContextualClient.ts
const elevatedContextualClient = createClient({
	auth: () => {
		const store = authAsyncLocalStorage.getStore();
		if (store?.elevatedAuth == null) throw new Error("Cannot find authentication on `AsyncLocalStorage` instance");
		return store.elevatedAuth;
	},
	host: hostProxy
});

//#region src/utils/generateVisitorTokens.ts
const auth = OAuthStrategy({ clientId: WIX_CLIENT_ID });
async function generateVisitorTokens() {
	return auth.generateVisitorTokens();
}

//#endregion
//#region ../../node_modules/astro/node_modules/zod/v3/helpers/util.js
var util;
(function(util) {
	util.assertEqual = (_) => {};
...
//#endregion
//#region ../../node_modules/astro/node_modules/zod/v3/ZodError.js
...
//#endregion
//#region ../../node_modules/astro/node_modules/zod/v3/locales/en.js
...
//#endregion
//#region ../../node_modules/astro/node_modules/zod/v3/errors.js
...
//#endregion
//#region ../../node_modules/astro/node_modules/zod/v3/helpers/parseUtil.js
...
//#endregion
//#region ../../node_modules/astro/node_modules/zod/v3/helpers/errorUtil.js
var errorUtil;
(function(errorUtil) {
...
//#endregion
//#region ../../node_modules/astro/node_modules/zod/v3/types.js
...
//#endregion
//#region src/utils/getSessionTokensFromCookie.ts
const tokensSchema = objectType({
	clientId: stringType(),
	tokens: objectType({
		...
		refreshToken: objectType({
			role: nativeEnumType(TokenRole),
			value: stringType()
		})
	})
});
function getSessionTokensFromCookie(context) {
	if (!context.isPrerendered) {
		...
		if (rawCookie != null) {
			...
			if (tokensParseResult.success && tokensParseResult.data.clientId === WIX_CLIENT_ID) return tokensParseResult.data.tokens;
		}
	}
	return null;
}
...
const onRequest$4 = async (context, next) => {
	...
	if (existingToken) return authAsyncLocalStorage.run({
		auth: OAuthStrategy({
	...
	if (context.request.headers.get("Accept")?.includes("text/html") && !context.isPrerendered) saveSessionTokensToCookie(context, visitorToken);
	return await authAsyncLocalStorage.run({
		auth: OAuthStrategy({
			clientId: WIX_CLIENT_ID,
			tokens: visitorToken
		}),
...
function getExistingTokens(context) {
	...
	if (existingTokens && existingTokens.accessToken.expiresAt + 600 > Date.now() / 1e3) return existingTokens;
	return null;
}
...
if (!global[key]) {
  global[key] = new AsyncLocalStorage$1();
}
