...
export const TextLoop = ({
  text = 'SANSKRITI ✦ LIVING CULTURE ✦ DISCOVER INDIA ✦ TIMELESS HERITAGE',
  shape = 'wave',
  path,
  speed = 85,
  direction = 'forward',
  ...
  // Default fallback metrics so animation starts immediately without waiting
  const [metrics, setMetrics] = useState<Metrics>({ length: 1600, reps: 4 });
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      let unitWidth = 0;
      try {
        length = pathEl.getTotalLength() || 1600;
        unitWidth = measureEl.getComputedTextLength() || 400;
      } catch {
        length = 1600;
        unitWidth = 400;
      }

      const reps = unitWidth > 0 ? Math.max(2, Math.round(length / unitWidth) * 2) : 4;
      setMetrics({ length, reps });
    };

    // Measure immediately and retry after font/DOM paint
    measure();
  ...
  const loopText = unit.repeat(metrics.reps || 4);
